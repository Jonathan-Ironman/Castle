...
export class MissionHelper {
  private static generateRewards(
    ...
    const rewardGold = new RewardGold(
      MathHelpers.getRandomInt(50, Math.max(50, teamStats.cost * 2))
    );
    const rewardReputation = new RewardReputation(
      MathHelpers.getRandomInt(1, 3)
    );
    return [rewardGold, rewardReputation];
  }

  private static generateAdversity(
    teamStats: TeamStats,
    playerReputation: number
  ): Adversity {
    return new Adversity({
      combat: MathHelpers.getRandomInt(
        (teamStats.combat / teamStats.heroCount) * 0.6,
        teamStats.combat * 1.5
      ),
      valor: MathHelpers.getRandomInt(
        (teamStats.valor / teamStats.heroCount) * 0.6,
        teamStats.combat * 1.5
      ),
      tactics: MathHelpers.getRandomInt(
        (teamStats.tactics / teamStats.heroCount) * 0.6,
        teamStats.combat * 1.5
      ),
      fear: 10,
      paranormal: false
  ...
  static generateMission(
    ...
    const adversity = MissionHelper.generateAdversity(
      teamStats,
      playerReputation
    );
    const rewards = MissionHelper.generateRewards(teamStats, playerReputation);
    const { title, description } = genericStories[
      MathHelpers.getRandomInt(0, genericStories.length - 1)
    ];
